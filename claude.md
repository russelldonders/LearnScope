...
Routes: `/`, `/login`, `/signup`, `/forgot-password`, `/reset-password`, `/welcome`, `/rate/:code`, `/recommend/:code`, `/providers/:slug`, `/dashboard`, `/profile`, `/connections`, `/actions`, `/help`.
...
Adding an unreleased Staging entry does **not** authorise a Production change or a numbered release — that still only happens via the `staging` → `master` merge below.
...
3. If step 2 ran migrations by hand rather than via `supabase db push`, Production's own migration-history table won't reflect them. Reconcile with `supabase migration repair --status applied --linked <versions>` while briefly linked to Production, then relink back to Staging immediately after — leaving this out doesn't break anything today, but it means `supabase migration list --linked` against Production silently stops being trustworthy.
4. Merge `staging` into `master` and push. Treat this as a deliberate release requiring explicit user confirmation each time, not a routine auto-push.

The push to `master` itself is what bundles and versions the release: `.github/workflows/release-platform-version.yml` runs `scripts/release-platform-version.mjs` on every push to `master`, which bundles whatever is currently pending in Staging's `platform_changelog_entries` into a new numbered `platform_releases` version there, then mirrors the same version number and entries onto Production as an already-released batch (via `system_confirm_platform_release`, the migration `20260922090000_system_confirm_platform_release.sql` added alongside `platform_releases`/`platform_changelog_entries`'s own schema in `20260913150000_platform_releases_and_changelog.sql`). This needs `SUPABASE_STAGING_URL`, `SUPABASE_STAGING_SERVICE_ROLE_KEY`, `SUPABASE_PRODUCTION_URL` and `SUPABASE_PRODUCTION_SERVICE_ROLE_KEY` set as GitHub Actions secrets, and the `20260922090000` migration applied to Production per step 2 above before it can run there. Nothing is confirmed by hand in the platform admin console (`/admin/releases`) any more — that page is read-only for versioning, showing pending entries and release history only.
