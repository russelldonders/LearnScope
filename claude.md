...
4. Add a "What's new" changelog entry (`platform_changelog_entries`, platform admin console → What's new, `/admin/releases`) for each user/admin-visible change in this batch, then confirm a new `platform_releases` version bundling them. Claude does this proactively as part of every release, without being asked — see the feature's own migration (`20260913150000_platform_releases_and_changelog.sql`) for the schema. Entries are plain-language and admin-facing, not dev commit messages. `platform_releases`/`platform_changelog_entries` are separate physical tables in each environment's own database (no automatic sync) — write the same entries and the same version number to **both** Staging and Production so their release histories and version numbers always match; never let one environment's version number outrun the other's.
5. Merge `staging` into `master` and push. Treat this as a deliberate release requiring explicit user confirmation each time, not a routine auto-push.
