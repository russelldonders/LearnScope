...
function TrainingAddForm({ items, selectedId, level, saving, onSelect, onLevel, onAdd, onCancel }) {
  ...
  const types = [...new Set(items.map((item) => item.course_type).filter(Boolean))].sort()
  const needle = query.trim().toLowerCase()
  const filtered = items.filter((item) => (type === 'all' || item.course_type === type) && (!needle || [item.name, item.course_code, item.course_type].filter(Boolean).some((value) => value.toLowerCase().includes(needle))))
  function updateFilters(nextQuery, nextType) {
    setQuery(nextQuery)
    setType(nextType)
    onSelect('')
  }
  return <div className="bg-card border border-hairline rounded-lg p-4 mb-4"><h3 className="font-display text-base text-ink mb-3">Add published training</h3>{items.length === 0 ? <p className="text-sm text-secondary">All published training is already aligned.</p> : <><div className="grid grid-cols-1 sm:grid-cols-[minmax(0,1fr)_220px] gap-3 mb-3"><label className="text-xs text-secondary">Search<input type="search" value={query} onChange={(event) => updateFilters(event.target.value, type)} placeholder="Search by name or code" className="mt-1 min-h-11 w-full rounded-md border border-hairline bg-paper px-3 text-sm text-ink placeholder:text-secondary focus:outline-none focus:ring-2 focus:ring-moss" /></label><label className="text-xs text-secondary">Training type<select value={type} onChange={(event) => updateFilters(query, event.target.value)} className="mt-1 min-h-11 w-full rounded-md border border-hairline bg-paper px-3 text-sm text-ink focus:outline-none focus:ring-2 focus:ring-moss"><option value="all">All types</option>{types.map((value) => <option key={value} value={value}>{value}</option>)}</select></label></div><p className="mb-2 text-xs text-secondary" aria-live="polite">{filtered.length} of {items.length} available</p>{filtered.length === 0 ? <p className="rounded-md border border-dashed border-hairline px-4 py-6 text-center text-sm text-secondary">No training matches these filters.</p> : <div className="grid grid-cols-1 sm:grid-cols-[minmax(0,1fr)_140px_auto] gap-3 sm:items-end"><label className="text-xs text-secondary">Training<select value={selectedId} onChange={(event) => onSelect(event.target.value)} className="mt-1 min-h-11 w-full rounded-md border border-hairline bg-paper px-3 text-sm text-ink focus:outline-none focus:ring-2 focus:ring-moss"><option value="">Choose training…</option>{filtered.map((item) => <option key={item.id} value={item.id}>{item.name}{item.course_code ? ` · ${item.course_code}` : ''}</option>)}</select></label><label className="text-xs text-secondary">Target level<select value={level} onChange={(event) => onLevel(Number(event.target.value))} className="mt-1 min-h-11 w-full rounded-md border border-hairline bg-paper px-3 text-sm text-ink focus:outline-none focus:ring-2 focus:ring-moss">{[1,2,3,4,5].map((value) => <option key={value} value={value}>{value}</option>)}</select></label><div className="flex gap-2"><button type="button" onClick={onAdd} disabled={!selectedId || saving} className="min-h-11 rounded-md bg-moss text-paper px-4 py-2 text-sm font-medium disabled:opacity-50">{saving ? 'Adding…' : 'Add'}</button><button type="button" onClick={onCancel} className="min-h-11 rounded-md border border-hairline px-3 py-2 text-sm text-ink">Cancel</button></div></div>}</>}</div>
}
...
function ResourceAddForm({ items, selectedId, saving, onSelect, onAdd, onCancel }) {
  ...
  const needle = query.trim().toLowerCase()
  const filtered = items.filter((item) => (type === 'all' || item.type === type) && (!needle || item.title.toLowerCase().includes(needle)))
  function updateFilters(nextQuery, nextType) {
    setQuery(nextQuery)
    setType(nextType)
    onSelect('')
  }
  return <div className="bg-card border border-hairline rounded-lg p-4 mb-4"><h3 className="font-display text-base text-ink mb-3">Add published resource</h3>{items.length === 0 ? <p className="text-sm text-secondary">All published resources are already aligned.</p> : <><div className="grid grid-cols-1 sm:grid-cols-[minmax(0,1fr)_220px] gap-3 mb-3"><label className="text-xs text-secondary">Search<input type="search" value={query} onChange={(event) => updateFilters(event.target.value, type)} placeholder="Search resources" className="mt-1 min-h-11 w-full rounded-md border border-hairline bg-paper px-3 text-sm text-ink placeholder:text-secondary focus:outline-none focus:ring-2 focus:ring-moss" /></label><label className="text-xs text-secondary">Resource type<select value={type} onChange={(event) => updateFilters(query, event.target.value)} className="mt-1 min-h-11 w-full rounded-md border border-hairline bg-paper px-3 text-sm text-ink focus:outline-none focus:ring-2 focus:ring-moss"><option value="all">All types</option>{types.map((value) => <option key={value} value={value}>{value.replaceAll('_', ' ')}</option>)}</select></label></div><p className="mb-2 text-xs text-secondary" aria-live="polite">{filtered.length} of {items.length} available</p>{filtered.length === 0 ? <p className="rounded-md border border-dashed border-hairline px-4 py-6 text-center text-sm text-secondary">No resources match these filters.</p> : <div className="grid grid-cols-1 sm:grid-cols-[minmax(0,1fr)_auto] gap-3 sm:items-end"><label className="text-xs text-secondary">Resource<select value={selectedId} onChange={(event) => onSelect(event.target.value)} className="mt-1 min-h-11 w-full rounded-md border border-hairline bg-paper px-3 py-2 text-sm text-ink focus:outline-none focus:ring-2 focus:ring-moss"><option value="">Choose a resource…</option>{filtered.map((item) => <option key={item.id} value={item.id}>{item.title} ({item.type.replaceAll('_', ' ')})</option>)}</select></label><div className="flex gap-2"><button type="button" onClick={onAdd} disabled={!selectedId || saving} className="min-h-11 rounded-md bg-moss text-paper px-4 py-2 text-sm font-medium disabled:opacity-50">{saving ? 'Adding…' : 'Add'}</button><button type="button" onClick={onCancel} className="min-h-11 rounded-md border border-hairline px-3 py-2 text-sm text-ink">Cancel</button></div></div>}</>}</div>
}
