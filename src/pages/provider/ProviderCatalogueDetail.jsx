...
function UsersTab({ catalogueId, members, orgMembers, canManage, userId, onReload, onError }) {
  ...
  const available = orgMembers.filter((member) => member.status !== 'pending' && !members.some((item) => item.user_id === member.user_id))
  ...
  return <section><SectionHeading title="Users" description="Catalogue admins manage content and access. Approvers review what is ready to publish." action={canManage && available.length ? { label: adding ? 'Cancel' : 'Add user', onClick: () => setAdding((value) => !value) } : null} />
    ...
    {members.length === 0 ? <EmptyState>No catalogue-specific users yet. Organisation admins still retain access.</EmptyState> : <ul className="divide-y divide-hairline border-y border-hairline">{members.map((member) => <li key={member.id} className="flex items-center justify-between gap-4 py-4"><div className="min-w-0"><p className="truncate font-medium text-ink">{emailByUser.get(member.user_id) || member.user_id}</p><p className="mt-1 text-sm capitalize text-secondary">Catalogue {member.role}</p></div>{canManage && <CogMenu label={`Manage ${emailByUser.get(member.user_id) || 'user'}`} options={[member.role === 'admin' ? { label: 'Make approver', action: () => changeRole(member, 'approver') } : { label: 'Make admin', action: () => changeRole(member, 'admin') }]} destructiveLabel="Remove from catalogue" onDestructive={() => remove(member.id)} />}</li>)}</ul>}
  </section>
}
