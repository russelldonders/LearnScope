...
export default function GrowthRing({ level, size = 64, showLabel = false }) {
  ...
  return (
    <div className="flex flex-col items-center gap-1">
      <svg width={size} height={size} viewBox="0 0 100 100" role="img" aria-label={`Level ${level}: ${LEVEL_LABELS[level] ?? ''}`}>
        {RADII.map((r, i) => {
      ...
      {showLabel && (
        <span className="font-mono text-xs text-secondary">{LEVEL_LABELS[level] ?? ''}</span>
      )}
    </div>
  )
}
